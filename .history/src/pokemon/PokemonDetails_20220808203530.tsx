...
import { getBackgroundColors } from "../services/getColors";
...
export function PokemonDetails() {
  ...
  const height1 = pokemonHeight >= 1;
  const height0 = pokemonHeight <= 0;

  return (
    <>
      <AppBarMenu />
      <Container>
        <Card className={styles.PokemonCard} key={pokemonSelectedDetails?.id} style={{ backgrondColor: 'green'}}>
          <Box
            display="flex"
            flexDirection="row"
            gap={2}
