...
export function PokemonDetails() {
  ...
  const pokemonType = pokemonSelectedDetails?.abilities;
  ...
  return (
    <>
      <AppBarMenu />
      <Container>
        <Card className={`styles.PokemonCard styles.Wave`} key={pokemonSelectedDetails?.id}>
          <Box
            display="flex"
            flexDirection="row"
            gap={2}
          ...
          >
            ...
            <Box display="flex" flexDirection="row" gap={0.6}>
              ...
              {pokemonSelectedDetails?.abilities.map((ability) => (
                <Typography className={styles.PokemonType} style={{ backgroundColor: setTypeColor(ability.ability.name) }}>
